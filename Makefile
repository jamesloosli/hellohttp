...
## docker: Build a new docker container from the local dockerfile.
docker:
	@-docker build . -t $(AUTHOR)/$(PROJECTNAME):$(VERSION) --build-arg BUILD=$(BUILD) --build-arg VERSION=$(VERSION)
